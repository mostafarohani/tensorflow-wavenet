...
def maxpool1d(x, k=2):
    ...
    x = tf.nn.max_pool(x, ksize=[1, 1, k, 1], strides=[
                       1, 1, k, 1], padding='SAME')  # 1, N, W, C
...
def net(x, weights, biases, dropout):
    ...
    fc1 = tf.reshape(
        conv2, [-1, weights['w3'].get_shape().as_list()[0]])  # (480)
...
def get_end_ops(fname_list, data_dict, coord, weights, biases):
    data_man = DataManager(fname_list, data_dict, coord,
                           SAMPLE_RATE, SECONDS_OF_AUDIO, N_CLASSES, 10 * BATCH_SIZE)
    x_batch, y_batch = data_man.dequeue(BATCH_SIZE)
    pred = net(x_batch, weights, biases, keep_prob)
    cost = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(pred, y_batch))
    auc_op, update_auc_op = tf.contrib.metrics.streaming_auc(
        tf.sigmoid(pred), y_batch)
