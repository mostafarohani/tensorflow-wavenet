...
class TestCausalConv(tf.test.TestCase):
    ...
    def testCausalConv(self):
        ...
        x1 = np.arange(1, 21, dtype=np.float16)
        x = np.append(x1, x1)
        x = np.reshape(x, [2, 20, 1])
        f = np.reshape(np.array([1, 1], dtype=np.float16), [2, 1, 1])
    ...
    def testNoTimeShift(self):
        ...
        x = np.arange(1, 11, dtype=np.float16)
        ...
        filter = np.reshape(np.array([0.0, 1.0], dtype=np.float16), [2, 1, 1])
